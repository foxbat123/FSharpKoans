    ...
    *)
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
            let rec innerF xs out =
              match List.length xs = List.length out with
              | true -> out
              | _ -> match xs with
                        | a::_ -> match xs with 
                                    |_::rest -> innerF rest (a::out)
                                    |_ -> out
                        |_ -> out
            innerF xs []  
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            let rec innerF sx out =
                match List.length xs = List.length out with
                | true -> out
                | _ -> match sx with
                        | a::_ -> match sx with 
                                    |_::rest -> innerF rest ((a + 1)::out)
                                    |_ -> out
                        |_ -> out
            innerF (List.rev xs) []  
   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            let rec innerF sx out =
                match List.length xs = 0 with
                | true -> out
                | _ -> match sx with
                        | a::_ -> match sx with 
                                    |_::rest -> innerF rest ((f a)::out)
                                    |_ -> out
                        |_ -> out
            innerF (List.rev xs) [] 
